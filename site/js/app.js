...
const app = $('#app');
const NAV = [['dashboard', 'لوحة المؤشرات'], ['projects', 'المشاريع'], ['tasks', 'المهام'], ['requests', 'الطلبات'], ['payments', 'المستخلصات'], ['report', 'التقرير التنفيذي'], ['ref', 'المرجع الفني'], ['admin', 'الإدارة', 'admin']];
...
async function route() {
  ...
  try {
    ...
    else if (parts[0] === 'report') { const { mountReport } = await import('./report.js'); await mountReport(m); }
