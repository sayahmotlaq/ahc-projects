...
export async function mountDashboard(root) {
  ...
  const subs = await q(sb.from('submittals').select('id,project_id,no,title,status,due_on,projects(name)').neq('status', 'decided'));
  const subsLate = subs.filter(subLate);
  const soon = new Date(); soon.setDate(soon.getDate() + 30); const docsExp = await q(sb.from('documents').select('id,project_id,category,title,expiry_date,projects(name)').in('category', ['bank_guarantee', 'insurance']).order('expiry_date'));
  const expiring = docsExp.filter(d => d.expiry_date && d.expiry_date <= soon.toISOString().slice(0, 10));
  ...
  root.innerHTML = `<div class="dash">
    <div class="kpis"><div class="kpi"><b>${projects.length}</b><span>إجمالي المشاريع</span></div><div class="kpi"><b>${active.length}</b><span>مشاريع قائمة</span></div><div class="kpi"><b>${exec.length}</b><span>تحت التنفيذ</span></div><div class="kpi ${late.length ? 'bad' : ''}"><b>${late.length}</b><span>متأخرة / متعثرة</span></div><div class="kpi ${chs.length ? 'bad' : ''}"><b>${chs.length}</b><span>تحديات مفتوحة</span></div><a class="kpi ${pays.length ? 'bad' : ''}" href="#/payments"><b>${payPend.length}<small> / ${payFin.length}</small></b><span>مستخلصات قيد الاعتماد / لدى المالية · ${money(Math.round(pays.reduce((a, r) => a + Number(r.net_amount || 0), 0)))} ر.س</span></a><a class="kpi ${subsLate.length ? 'bad' : ''}" href="#/submittals"><b>${subs.length}</b><span>اعتمادات قيد الإجراء${subsLate.length ? ' · ' + subsLate.length + ' متأخرة' : ''}</span></a><a class="kpi ${trs.length ? 'bad' : ''}" href="#/treports"><b>${trs.length}</b><span>تقارير فنية بانتظار المراجعة</span></a><a class="kpi ${lateTasks.length ? 'bad' : ''}" href="#/tasks"><b>${tasks.length}</b><span>مهام مفتوحة${lateTasks.length ? ' · ' + lateTasks.length + ' متأخرة' : ''}</span></a><a class="kpi ${reqs.length ? 'bad' : ''}" href="#/requests"><b>${reqs.length}</b><span>طلبات بانتظار الرد</span></a><a class="kpi ${stale.length ? 'bad' : ''}" href="#/projects?g=all&flag=stale"><b>${stale.length}</b><span>بلا تحديث منذ ${STALE_DAYS} يوماً</span></a><div class="kpi"><b>${totalV >= 1e6 ? (totalV / 1e6).toFixed(1) + '<small> مليون</small>' : fmt0(totalV)}</b><span>القيمة الإجمالية (ر.س)</span></div></div>
    ...
    ${expiring.length ? `<div class="pcard"><h2><span class="ic"></span>ضمانات وتأمينات تنتهي خلال 30 يوماً</h2><table class="lst"><thead><tr><th>المشروع</th><th>المستند</th><th>الانتهاء</th></tr></thead><tbody>${expiring.map(d => `<tr data-open="${d.project_id}"><td><b>${esc(d.projects?.name || '')}</b></td><td>${d.category === 'bank_guarantee' ? 'ضمان بنكي' : 'وثيقة تأمين'} — ${esc(d.title)}</td><td class="${d.expiry_date < today() ? 'bad' : ''}">${dateAr(d.expiry_date)}${d.expiry_date < today() ? ' — منتهٍ' : ''}</td></tr>`).join('')}</tbody></table></div>` : ''}
    <div class="two">
...
export async function mountProject(root, id, tab = 'overview', sub) {
  ...
  const tabs = [['overview', 'نظرة عامة'], ['tasks', 'المهام'], ['requests', 'الطلبات'], ['payments', 'المستخلصات'], ['treports', 'التقارير الفنية'], ['docs', 'المستندات'], ['drawings', 'المخططات'], ['submittals', 'الاعتمادات'], ['challenges', 'التحديات والمخاطر'], ['boq', 'جداول الكميات'], ['updates', 'التحديثات والملاحظات'], ['log', 'سجل المراحل']];
