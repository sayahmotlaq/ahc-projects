...
export async function mountDashboard(root) {
  ...
  root.innerHTML = `<div class="dash">
    <div class="kpis"><div class="kpi"><b>${projects.length}</b><span>إجمالي المشاريع</span></div><div class="kpi"><b>${active.length}</b><span>مشاريع قائمة</span></div><div class="kpi"><b>${exec.length}</b><span>تحت التنفيذ</span></div><div class="kpi ${late.length ? 'bad' : ''}"><b>${late.length}</b><span>متأخرة / متعثرة</span></div><div class="kpi ${chs.length ? 'bad' : ''}"><b>${chs.length}</b><span>تحديات مفتوحة</span></div><a class="kpi ${lateTasks.length ? 'bad' : ''}" href="#/tasks"><b>${tasks.length}</b><span>مهام مفتوحة${lateTasks.length ? ' · ' + lateTasks.length + ' متأخرة' : ''}</span></a><a class="kpi ${reqs.length ? 'bad' : ''}" href="#/requests"><b>${reqs.length}</b><span>طلبات بانتظار الرد</span></a><a class="kpi ${stale.length ? 'bad' : ''}" href="#/projects?g=all&flag=stale"><b>${stale.length}</b><span>بلا تحديث منذ ${STALE_DAYS} يوماً</span></a><div class="kpi"><b>${totalV >= 1e6 ? (totalV / 1e6).toFixed(1) + '<small> مليون</small>' : fmt0(totalV)}</b><span>القيمة الإجمالية (ر.س)</span></div></div>
...
export async function mountProject(root, id, tab = 'overview', sub) {
  ...
  const tabs = [['overview', 'نظرة عامة'], ['tasks', 'المهام'], ['requests', 'الطلبات'], ['challenges', 'التحديات والمخاطر'], ['boq', 'جداول الكميات'], ['updates', 'التحديثات والملاحظات'], ['log', 'سجل المراحل']];
