...
async function subForm(s, p, existing, done, resubmitOf = null) {
  ...
  await modal(`<form id="f" class="pgrid">
    ...
    <div class="fld wide"><span>البند المعني</span><div class="seg" id="specmode"><label><input type="radio" name="spec_source" value="boq" ${(v.spec_source || (v.boq_line_id || !s && !resubmitOf ? 'boq' : 'ref')) === 'boq' ? 'checked' : ''}> من جدول كميات المشروع</label><label><input type="radio" name="spec_source" value="ref" ${(v.spec_source || (v.boq_line_id || !s && !resubmitOf ? 'boq' : 'ref')) === 'ref' ? 'checked' : ''}> بحث في المرجع الفني</label><label><input type="radio" name="spec_source" value="free" ${v.spec_source === 'free' ? 'checked' : ''}> كتابة حرة</label></div>
      <div id="specBoq"><select name="boq_line_id" id="boqsel"><option value="">جارٍ تحميل بنود المشروع…</option></select></div>
      <div id="specRef" class="specdrop-wrap"><input id="specq" value="${esc(v.spec_ref && v.spec_source !== 'free' ? v.spec_ref + ' — ' + (v.spec_title || '') : '')}" placeholder="اكتب اسم البند أو رقمه — مثال: بلاط بورسلين أو 09 30 13" autocomplete="off"><div class="specdrop" id="specdrop"></div></div>
      <div id="specFree"><input id="specfree" value="${esc(v.spec_source === 'free' ? (v.spec_title || '') : '')}" placeholder="وصف البند كما ورد في العقد"></div>
      <input type="hidden" name="spec_ref" value="${esc(v.spec_ref || '')}"><input type="hidden" name="spec_title" value="${esc(v.spec_title || '')}"></div>
    ...
    <div class="btnrow end wide">${s && isAdmin() ? '<button type="button" class="btn danger" data-del>حذف</button>' : ''}<span style="flex:1"></span><button type="button" class="btn" data-x>إلغاء</button><button class="btn primary">${resubmitOf ? 'تسجيل إعادة التقديم' : 'حفظ'}</button></div></form>`, { title: resubmitOf ? 'إعادة تقديم' : s ? 'تعديل طلب الاعتماد' : 'طلب اعتماد جديد', wide: true, onOpen: async (w, close) => {
      const qi = $('#specq', w), dd = $('#specdrop', w), refIn = w.querySelector('[name=spec_ref]'), titIn = w.querySelector('[name=spec_title]');
      const mode = () => w.querySelector('[name=spec_source]:checked').value;
      const tog = () => { const m = mode(); $('#specBoq', w).style.display = m === 'boq' ? '' : 'none'; $('#specRef', w).style.display = m === 'ref' ? '' : 'none'; $('#specFree', w).style.display = m === 'free' ? '' : 'none'; };
      $$('[name=spec_source]', w).forEach(r => r.onchange = tog); tog();
      // بنود جداول كميات المشروع
      (async () => { try { const boqs = await q(sb.from('boqs').select('id,name,status').eq('project_id', p.id).order('created_at')); const sel0 = $('#boqsel', w); if (!boqs.length) { sel0.innerHTML = '<option value="">لا توجد جداول كميات لهذا المشروع — استخدم البحث في المرجع</option>'; if (mode() === 'boq' && !v.boq_line_id) { w.querySelector('[name=spec_source][value=ref]').checked = true; tog(); } return; }
        if (!REF.loaded) { try { await loadRef(false); } catch (e) { } }
        const lines = await q(sb.from('boq_lines').select('id,boq_id,variant_code,qty,loc').in('boq_id', boqs.map(b => b.id)).order('sort').order('id'));
        const lbl = l => { const vr = REF.byVar?.[l.variant_code]; const it = vr?.item; return `${it ? it.code + ' — ' + it.ar : l.variant_code}${vr && vr.ar && it && vr.ar !== it.ar ? ' (' + vr.ar + ')' : ''} × ${Number(l.qty)} ${it?.unit || ''}${l.loc ? ' — ' + l.loc : ''}`; };
        sel0.innerHTML = '<option value="">— اختر البند —</option>' + boqs.map(b => `<optgroup label="${esc(b.name)}">${lines.filter(l => l.boq_id === b.id).map(l => `<option value="${l.id}" ${String(v.boq_line_id) === String(l.id) ? 'selected' : ''}>${esc(lbl(l))}</option>`).join('')}</optgroup>`).join('');
        sel0.onchange = () => { const l = lines.find(x => String(x.id) === sel0.value); if (!l) return; const vr = REF.byVar?.[l.variant_code]; const it = vr?.item; refIn.value = it?.code || ''; titIn.value = (it ? it.ar : l.variant_code) + (vr && vr.ar && it && vr.ar !== it.ar ? ' — ' + vr.ar : ''); };
      ...
      qi.onchange = () => { if (!qi.value.trim()) { refIn.value = ''; titIn.value = ''; } };
      $('#specfree', w).oninput = () => { refIn.value = ''; titIn.value = $('#specfree', w).value.trim(); };
      $('#f', w).onsubmit = async e => { e.preventDefault(); const f = formData(e.target); const btn = e.target.querySelector('button.primary'); btn.disabled = true;
        try { const due = f.due_on || (() => { const d = new Date(f.submitted_on || today()); d.setDate(d.getDate() + SLA); return d.toISOString().slice(0, 10); })();
          const m = f.spec_source; if (m === 'boq' && !f.boq_line_id) { btn.disabled = false; return err('اختر البند من جدول الكميات أو غيّر طريقة التحديد'); }
          const row = { project_id: p.id, kind: f.kind, title: f.title.trim(), description: f.description.trim(), spec_source: m, boq_line_id: m === 'boq' ? Number(f.boq_line_id) : null, spec_ref: m === 'free' ? '' : f.spec_ref, spec_title: m === 'free' ? $('#specfree', w).value.trim() : f.spec_title, submitted_by: f.submitted_by.trim(), submitted_on: f.submitted_on || today(), due_on: due };
          let sid = s?.id;
