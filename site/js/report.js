...
export async function mountReport(root) {
  ...
  const [projects, profiles, act, chs, tasks, reqs, ups] = await Promise.all([
    ...
    q(sb.from('project_updates').select('*, projects(name)').order('created_at', { ascending: false }).limit(6)),
  ]);
