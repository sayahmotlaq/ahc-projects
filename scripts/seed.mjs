...
async function upsert(table, rows, chunk = 300) {
  ...
  for (let i = 0; i < rows.length; i += chunk) {
    ...
    const r = await fetch(`${BASE}/rest/v1/${table}?on_conflict=code`, { method: 'POST', headers: { ...H, 'Prefer': 'resolution=merge-duplicates,return=minimal' }, body: JSON.stringify(part) });
...
async function fetchAll(table, select) {
  ...
  while (true) {
    const r = await fetch(`${BASE}/rest/v1/${table}?select=${select}&order=code`, { headers: { ...H, 'Range': `${from}-${from + step - 1}` } });
...
const r = await fetch(`${BASE}/rest/v1/settings?key=eq.ref_version`, { method: 'PATCH', headers: { ...H, 'Prefer': 'return=minimal' },
